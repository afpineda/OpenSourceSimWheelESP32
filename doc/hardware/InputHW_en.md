...
The market's typical line decoders use addresses that are 3 bits long (decoding to 8 lines).
One of these would allow you to use just six pins to achieve 24 inputs.

The logic behavior of an encoded button matrix is exactly the same
as that of analog multiplexers (see below),
but the circuit is a bit more difficult to build.
That is why it is not recommended.
However, if you wish to use this circuit,
configure the firmware as if it were analog multiplexers and it will work.
