...
as long as it is based on Expressif's "ESP32" architecture
and features BLE support.
Expressif's "ESP32C3" boards are not recommended
...
- Those designed and sold by
  [Unexpected Maker](https://unexpectedmaker.com/shop?category=Development+Boards),
  like "Feather S3", "Tiny S3" or "TinyPico".
  Those are open hardware, as well.
  - The [OMGS3](https://esp32s3.com/omgs3.html)
    DevKit board features a built-in "fuel gauge",
    which is supported by this project.
    Unfortunately, that board is for surface-mounting only.
- [Adafruit Feather 32u4 Bluefruit LE](https://www.adafruit.com/product/2829).
- [Wemos boards](https://www.wemos.cc/). *Note*:
  - The "D32" series are also known as "Lolin32" and
    found by that name at many retailers.
  - Wemos "Lolin32 Lite" is an outdated and deprecated board.
    However, very cheap clones are still sold at some retailers.
  - "S3 mini" is a very interesting board
    if you don't need built-in battery support.
- [Sparkfun Thing Plus](https://www.sparkfun.com/products/17381).
- [Some LilyGO boards](http://www.lilygo.cc/)
  even if their built-in display is not used.
...
This project makes extensive use of the official
"ESP32-DevKit-C" and "ESP32S3-DevKit-C" boards
...
> [custom DevKit board](../pcb/CustomDevKit/CustomDevKit.md)
...
this project requires 50%-80% of the flash memory
capacity of a "standard" ESP32 DevKit, which is **4 MB**.
However, the firmware size may increase further.
In case of extreme need,
you may still reconfigure the partition table
to make more room for the firmware:
...
you need to know which constraints apply to your DevKit board.
Look for a data sheet.
...
- **GPIO #1 and #3**:
  ...
  - These pins are used by the USB serial port.
    They should be reserved for firmware upload and
...
- [Pin-out](https://www.luisllamas.es/en/esp32-s3-hardware-details-pinout/)
...
- [Pin-out](https://www.studiopieters.nl/esp32-c3-pinout/)
- [C3-DevKit-C](https://docs.espressif.com/projects/esp-dev-kits/en/latest/esp32c3/esp32-c3-devkitc-02/index.html).

## Uploading firmware to your DevKit board

In order to upload a firmware to a DevKit board,
certain pins, called "bootstrap" pins, are used.
...
In Arduino IDE, you need to configure the "board manager"
with the proper parameters.
Most times, you only need to select the correct board and
...
Some DevKit boards refuses to upload firmware unless
you put them in "bootloader mode".
...
If your DevKit board does not feature a "boot" button,
drive `GPIO #0` to `GND` instead.

If your DevKit board does not feature a "reset" button,
drive `EN` to `GND` instead.

It is said that some boards require a small capacitor
between `EN` and `GND` pins to avoid this annoyance.
